...
interface Definition { name: string; start: Vec3; role: Role; settings: RoomSettings; segments: { ticks: number; input: Partial<Input> }[] }
const definitions: Definition[] = [
  {name:'switchyard-crawl-headroom',start:{x:-26,y:0,z:-17},role:'hider',settings:{...DEFAULT_SETTINGS,mapId:'switchyard'},segments:[{ticks:45,input:{mx:1,crouch:true}},{ticks:20,input:{mx:1}},{ticks:70,input:{mx:1}}]},
  {name:'glassworks-auto-hop',start:{x:-50,y:0,z:49},role:'hider',settings:{...DEFAULT_SETTINGS,mapId:'glassworks',bunnyHop:'auto'},segments:[{ticks:60,input:{mx:1}},{ticks:90,input:{mx:1,jump:true}},{ticks:100,input:{mx:1,mz:1,yaw:1.1,jump:true}}]},
...
const cases = definitions.map(d => {
  ...
  return {...d,segments:d.segments.map(s=>{
    const input={...neutralInput(),...s.input};for(let i=0;i<s.ticks;i++)move(motor,input,d.role,1/60,layout.boxes,layout.half,d.settings);
    return {ticks:s.ticks,input,expected:{...motor}};
  })};
});
const path='tests/fixtures/gameplay.json';
const text=JSON.stringify({description:'Generated from TypeScript movement. Verify with the Rust parity test; do not edit expected values.',cases})+'\n';
if(process.argv.includes('--check')){if(readFileSync(path,'utf8')!==text)throw new Error('Run npm run fixtures, review the results and run cargo test.');console.log('Gameplay fixtures match.');}
else{writeFileSync(path,text);console.log(`Wrote ${cases.length} gameplay scenarios.`);}
