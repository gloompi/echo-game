...
export function installHooks(root, env) {
  ...
  const inside = spawnSync('git', ['rev-parse', '--is-inside-work-tree'], { cwd: root, encoding: 'utf8', env });
  if (inside.status !== 0 || inside.stdout.trim() !== 'true') return 'Not a Git working tree; hooks were not installed.';
  const current = spawnSync('git', ['config', '--get', 'core.hooksPath'], { cwd: root, encoding: 'utf8', env });
  ...
  const result = spawnSync('git', ['config', '--local', 'core.hooksPath', '.githooks'], { cwd: root, env });
  if (result.error || result.status !== 0) throw new Error('Could not install repository-local hooks.');
  return 'Installed repository-local Echo hooks (.githooks).';
}

if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  try {
    console.log(installHooks(resolve(dirname(fileURLToPath(import.meta.url)), '../..'), process.env));
  } catch (error) {
