...
export function importViolation(file, specifier) {
  ...
  if (file.startsWith('shared/'))
    return 'Shared simulation code must not import runtime/framework packages.';
  if (specifier.startsWith('node:') || nodeModules.has(specifier))
    return 'Browser code must not import Node built-ins.';
...
export function lintSource(file, text) {
  ...
  const visit = (node) => {
    if (ts.isDebuggerStatement(node))
      report(node.getStart(source), 'no-debugger', 'Remove debugger statements.');
    ...
    if (
      ...
      report(
        ...
        'no-dynamic-code',
        'Do not evaluate dynamically constructed code.',
      );
    }
    ...
    if (specifier) {
      if (ts.isStringLiteralLike(specifier)) {
        ...
        report(
          node.getStart(source),
          'architecture',
          'Production dynamic imports must use literal paths.',
        );
      }
    }
  ...
  for (let token = scanner.scan(); token !== ts.SyntaxKind.EndOfFileToken; token = scanner.scan()) {
    if (
      ...
      report(
        scanner.getTokenPos(),
        'no-typecheck-bypass',
        'Fix the type error; use an explained @ts-expect-error only in negative tests.',
      );
    }
  }
  return violations;
...
export function lintProject(root) {
  const files = ['client', 'shared', 'scripts', 'tests'].flatMap((dir) =>
  ...
  const violations = files.flatMap((file) =>
    lintSource(relative(root, file).replaceAll('\\', '/'), readFileSync(file, 'utf8')),
  );
  for (const violation of violations) {
    console.error(`${violation.file}:${violation.line} [${violation.rule}] ${violation.message}`);
  }
  console.log(
    `Checked ${files.length} JS/TS files: ${violations.length} rule violations, ${syntaxErrors.length} syntax errors.`,
  );
