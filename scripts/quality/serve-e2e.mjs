...
export function e2eEnvironment(inherited, root) {
  ...
  for (const key of Object.keys(env)) {
    if (key.startsWith('ECHO_') || ['HOST', 'PORT', 'ALLOWED_ORIGINS', 'MAX_ROOMS'].includes(key)) delete env[key];
  }
  return {
    ...env,
    HOST: '127.0.0.1', PORT: '3107',
    ECHO_WT_HOST: '127.0.0.1', ECHO_WT_PORT: '4447',
    ECHO_ACCESS_KEY: '', ECHO_PUBLIC_URL: '', ECHO_WT_PUBLIC_URL: '',
    ECHO_DELAY_MS: '3000', ECHO_CLIENT_DIR: join(root, 'dist/client'),
    ALLOWED_ORIGINS: 'http://127.0.0.1:3107', MAX_ROOMS: '32',
  };
}

/** @param {string} root @returns {void} */
function serve(root) {
  const binary = join(root, `target/release/echo-server${process.platform === 'win32' ? '.exe' : ''}`);
  if (!existsSync(binary) || !existsSync(join(root, 'dist/client/index.html'))) {
    throw new Error('Production build missing. Run pnpm test:e2e, or pnpm build before test:e2e:built.');
  }
  ...
  const child = spawn(binary, [], { cwd: workdir, env: e2eEnvironment(process.env, root), stdio: 'inherit' });
