...
function box(m, id, x, z, w, d, h, y = 0, kind = 'cover') {
  m.boxes.push({ id, x, z, w, d, h, y, kind });
}
function landmark(m, name, x, z) {
  m.landmarks.push({ name, x, y: 3.8, z });
}
function perimeterSpawns(m) {
  const edge = m.half - 5;
  for (const x of [-edge * 0.55, 0, edge * 0.55]) {
    m.spawns.push({ x, y: 0, z: -edge }, { x: -x, y: 0, z: edge });
    m.spawns.push({ x: -edge, y: 0, z: x }, { x: edge, y: 0, z: -x });
  }
  for (const x of [-edge, 0, edge])
    for (const z of [-edge, 0, edge]) if (x || z) m.waypoints.push({ x, y: 0, z });
}
function mirrors(m, name, ax, az, bx, bz) {
  for (const [id, target, x, z] of [
    [name + '-a', name + '-b', ax, az],
    [name + '-b', name + '-a', bx, bz],
  ]) {
...
function loop(m, prefix, cx, cz, turn = 0) {
  ...
  for (const z of [-5, 5])
    for (const x of [-3.75, 3.75]) local(`panel-${x}-${z}`, x, z, 4.5, 0.7, 3.4);
  local('window-sill', 0, 5, 3, 0.7, 0.55);
  local('window-lintel', 0, 5, 3, 0.7, 0.4, 3);
  for (const z of [-3.25, 3.25]) local(`east-${z}`, 6, z, 0.7, 3.5, 3.4);
  for (const z of [-3, 3]) local(`west-${z}`, -6, z, 0.7, 4, 3.4);
  local('crawl-lintel', -6, 0, 0.7, 2, 2.15, 1.25);
  local('crate', 1.8, 0, 2.1, 2.1, 1.4);
  local('alcove-back', -2.8, 2.8, 2, 0.25, 2.6);
  local('alcove-left', -3.75, 2, 0.25, 1.8, 2.6);
  local('alcove-right', -1.85, 2, 0.25, 1.8, 2.6);
  local('low-barricade', 0, 8, 4, 0.65, 0.65);
  const gx = cx,
    gz = cz - 8;
  m.waypoints.push(
    { x: gx, y: 0, z: gz },
...
function loft(m, id, x, z) {
  ...
  for (let i = 0; i < 6; i++)
    box(m, id + '-step-' + i, x - 2, z + 6.5 - i * 0.75, 2.3, 0.78, (i + 1) * 0.35, 0, 'step');
}
const classic = create(
  'afterhours',
  'Afterhours',
  23,
  '2–6 players',
  'Compact neon arena. Short loops, balconies and a diagonal mirror pair.',
  'neon',
);
...
const yard = create(
  ...
  40,
  '4–10 players',
  'Four warehouse loops, crawl exits, hiding alcoves and two flanking lofts.',
  'industrial',
);
...
for (const [x, z, r, n] of [
  ...
  loop(yard, n.toLowerCase().replaceAll(' ', '-'), x, z, r);
  landmark(yard, n, x, z - 7);
}
loft(yard, 'west-loft', -31, 0);
loft(yard, 'east-loft', 31, 0);
box(yard, 'central-crate', 0, 0, 3, 3, 1.5);
box(yard, 'central-cover', 0, -5, 6, 0.8, 2.5);
mirrors(yard, 'violet', -34, -32, 34, 32);
mirrors(yard, 'cyan', 34, -32, -34, 32);
const glass = create(
  'glassworks',
  'Glassworks',
  56,
  '8–12 players',
  'Large garden courtyards: hedge loops, recessed hiding pockets, elevated overlooks and mirror shortcuts.',
  'garden',
);
perimeterSpawns(glass);
for (const [x, z, r, n] of [
  [-28, -28, 0, 'FERN HOUSE'],
  [28, -28, 1, 'EAST GALLERY'],
  [28, 28, 2, 'SUN COURT'],
  [-28, 28, 3, 'WEST ATRIUM'],
]) {
  loop(glass, n.toLowerCase().replaceAll(' ', '-'), x, z, r);
  landmark(glass, n, x, z - 7);
...
for (const [x, z, id] of [
  ...
  loft(glass, id, x, z);
  landmark(glass, id.toUpperCase().replaceAll('-', ' '), x, z - 3);
}
...
for (const s of [-1, 1]) {
  ...
  box(glass, 'ring-low-' + s, s * 4, 0, 2, 3, 0.7);
}
mirrors(glass, 'orchid', -49, -46, 49, 46);
mirrors(glass, 'azure', 49, -46, -49, 46);
...
if (process.argv.includes('--check')) {
  if (readFileSync(file, 'utf8') !== text)
    throw new Error('Run node scripts/build-maps.mjs and commit shared/maps.json');
