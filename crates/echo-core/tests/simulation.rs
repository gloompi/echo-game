...
#[test]fn fire_rate_magazine_and_role_are_authoritative(){let mut r=pair(3000);r.player_mut("h").unwrap().motor.x=14.0;r.fire(0,NOW);r.fire(0,NOW+1.0);assert_eq!(r.player("s").unwrap().ammo,11);for i in 1..20{r.fire(0,NOW+i as f64*r.settings.balance.weapons.blaster.interval_ms as f64);}assert_eq!(r.player("s").unwrap().ammo,0);r.fire(1,NOW);assert_eq!(r.player("h").unwrap().ammo,12);}
#[test]fn two_hits_capture_and_finish(){let mut r=pair(3000);r.fire(0,NOW);let interval=r.settings.balance.weapons.blaster.interval_ms as f64;r.fire(0,NOW+interval);assert!(!r.player("h").unwrap().alive);r.tick(NOW+interval+1.0);assert_eq!(r.winner,Some(Role::Seeker));let ammo=r.player("s").unwrap().ammo;r.fire(0,NOW+500.0);assert_eq!(r.player("s").unwrap().ammo,ammo);}
