...
impl History {
    ...
    pub fn record(&mut self, at: f64, poses: Vec<Pose>) {
        ...
        // Keep one bracket just before the horizon for interpolation.
    ...
    pub fn sample(&self, at: f64) -> Vec<Pose> {
        ...
        if at < first.0 { return vec![]; } // Never clamp a missing past to a future/live pose.
        let Some(last) = self.frames.back() else { return vec![]; };
        if at >= last.0 { return last.1.clone(); }
        // Binary search over the ring; no O(history length) scan per viewer.
        ...
        let (ta,a)=&self.frames[low]; let (tb,b)=&self.frames[high];
        let t=((at-ta)/(tb-ta).max(0.001)).clamp(0.0,1.0);
        a.iter().map(|p| {
            let Some(q)=b.iter().find(|q|q.id==p.id) else {return p.clone();};
            if p.role!=q.role || p.alive!=q.alive {return p.clone();}
...
mod tests {
    ...
    fn retained_frames_are_bounded() {
        ...
        for tick in 0..10_000 { history.record(tick as f64 * 1000.0 / rules().tick_rate as f64, vec![]); }
