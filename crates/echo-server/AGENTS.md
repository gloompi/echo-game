...
Never hold the engine mutex across network I/O. Make shutdown, task cancellation, reader/writer ownership, and peer removal explicit. Keep `main.rs` as a small executable entry point. `runtime` owns startup/shutdown, `config` parses settings, `http` owns HTTP/static routes, `protocol` defines client commands, `security` owns endpoint/origin policies, `commands` orchestrates rooms, `sessions` owns stream I/O, and `simulation` schedules core ticks.
