...
mod tests {
    ...
    async fn frames_roundtrip() {
        ...
        let task = tokio::spawn(async move {
            write_frame(&mut tx, "{\"name\":\"Куба\"}", MAX_CONTROL)
                .await
                .unwrap();
        });
        assert_eq!(read_frame(&mut rx).await.unwrap(), "{\"name\":\"Куба\"}");
        task.await.unwrap();
    }
    #[tokio::test]
    async fn reject_length_before_body_allocation() {
    ...
    #[tokio::test]
    async fn reject_truncated_and_invalid_utf8() {
