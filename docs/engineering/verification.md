...
Use Node 22 (at least 22.12), the `pnpm@10.23.0` declared in `package.json`, and Rust stable with rustfmt and Clippy. Dependency installations must use genuine checked-in lockfiles.
...
| `pnpm lint:ts` | JS/TS syntax and custom AST safety/import-boundary rules |
...
Playwright captures screenshots/traces on failure and browser exceptions/console errors from both players. CI retains `test-results/` and `playwright-report/` for seven days. Do not attach secrets or hidden Hider positions to diagnostics.

## Current limitations

See `baseline.md` for executed evidence. Full locked dependency checks and the actual app could not run in the authoring environment. Existing compressed Rust files may require a repository-wide rustfmt pass and Clippy fixes; do not turn off these gates to merge. Broader JS typed ESLint/style formatting, legacy checkJs migration, further entry-point decomposition, and fully pinned toolchain/action revisions remain follow-up work requiring real installation/execution evidence.
