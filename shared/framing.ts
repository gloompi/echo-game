...
export function encodeFrame(text: string, maximum = MAX_CONTROL_BYTES): Uint8Array {
  ...
  if (!payload.length || payload.length > maximum)
    throw new Error('Frame exceeds the protocol limit.');
...
export class FrameDecoder {
  ...
  push(chunk: Uint8Array): string[] {
    ...
    while (offset < chunk.length) {
      if (!this.body) {
        ...
        this.header.set(chunk.subarray(offset, offset + n), this.headerUsed);
        this.headerUsed += n;
        offset += n;
      ...
      this.body.set(chunk.subarray(offset, offset + n), this.bodyUsed);
      this.bodyUsed += n;
      offset += n;
  ...
  finish(): void {
    if (this.headerUsed || this.body) throw new Error('Truncated frame.');
  }
}
