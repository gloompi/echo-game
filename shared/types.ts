...
export interface Input {
  ...
  crouch: boolean; interact: boolean; ability?: boolean; mine?: boolean; scan?: boolean; weapon?: Weapon;
}
export interface Motor extends Vec3 {
  ...
  slideTime?: number; slideCooldown?: number; crouchHeld?: boolean;
  controlLeft?: number; pullLeft?: number; pullX?: number; pullZ?: number; pullSpeed?: number;
}
export interface Pose extends Vec3 {
  ...
  crouched?: boolean; warp?: number; sliding?: boolean; shielded?: boolean; controlled?: boolean;
  hitAt?: number; skin?: Skin; weapon?: Weapon;
}
export interface PublicPlayer {
  id: string; name: string; role: Role; preference: Preference; bot: boolean;
  alive: boolean; tags: number; baits: number; caught: boolean; skin?: Skin;
}
export interface AbilityState {
  shieldLeft: number; shieldCooldown: number; mineCooldown: number; hookCooldown: number;
  scanLeft: number; scanCooldown: number; immuneLeft: number; switchLeft: number; teleportCastLeft: number;
}
export interface Self extends Motor {
  ...
  skin?: Skin; weapon?: Weapon; abilities?: AbilityState;
}
export interface GameEvent {
  id: number; at: number; kind: 'shot' | 'tag' | 'catch' | 'wave' | 'dash' | 'round' | 'teleport' | 'shield' | 'hook' | 'web' | 'mine' | 'scan' | 'control';
  actor: string; target?: string; from?: Vec3; to?: Vec3; hit?: boolean; echo?: boolean;
}
export interface MineView extends Vec3 { id: number; owner: string; armed: boolean }
export interface ProjectileView extends Vec3 { id: number; owner: string; radius: number }
export interface Snapshot {
  ...
  mines?: MineView[]; projectiles?: ProjectileView[];
}
export type ClientMessage =
  | { type: 'join'; mode: 'create' | 'join' | 'quick' | 'practice'; name: string; room?: string; preference?: Preference; accessKey?: string; settings?: RoomSettings; skin?: Skin }
...
export const neutralInput = (seq = 0): Input => ({
  ...
  ability: false, mine: false, scan: false,
});
