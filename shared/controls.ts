...
export function bindable(action: Action, code: string): boolean {
  ...
  return !reserved.has(code) && (/^Key[A-Z]$/.test(code) || /^Digit[0-9]$/.test(code) || ['Space','ControlLeft','AltLeft'].includes(code)
...
export function readBindings(raw: string | null): Bindings {
  try {
    ...
    if (typeof b.jump !== 'string' || typeof b.crouch !== 'string' || !bindable('jump',b.jump) || !bindable('crouch',b.crouch)) return { ...DEFAULT_BINDINGS };
...
export const keyName = (code: string) => ({ Space:'Space', ControlLeft:'Ctrl', AltLeft:'Alt', WheelDown:'Wheel ↓', WheelUp:'Wheel ↑' } as Record<string,string>)[code] ?? code.replace(/^Key|^Digit/,'');
