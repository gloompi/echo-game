...
export class CombatFX {
  ...
  private ringGeometry = new T.RingGeometry(0.985, 1, 64);
  private ringMaterial = new T.MeshBasicMaterial({
    color: 0x6ceede,
    transparent: true,
    opacity: 0.4,
    side: T.DoubleSide,
  ...
  private mineGeometry = new T.CylinderGeometry(0.33, 0.43, 0.1, 12);
  private mineMaterial = new T.MeshStandardMaterial({
    color: 0xde823c,
    emissive: 0xd77732,
  ...
  update(
    ...
    if (snapshot !== this.snapshotRef) {
      ...
      this.sync(
        ...
        snapshot.mines ?? [],
        scene,
        () => new T.Mesh(this.mineGeometry, this.mineMaterial),
        (mesh, value) => {
          mesh.position.set(value.x, value.y + 0.07, value.z);
          mesh.scale.setScalar('armed' in value && value.armed ? 1 : 0.7);
        },
      );
      this.sync(
        this.webs,
        ...
        scene,
        () => new T.Mesh(this.webGeometry, this.webMaterial),
        (mesh, value) => {
          mesh.position.set(value.x, value.y, value.z);
          mesh.scale.setScalar('radius' in value ? Number(value.radius) : 0.18);
        },
      );
    }
    for (const mesh of this.webs.values()) mesh.rotation.y = now / 180;
    const active =
      snapshot.self.role === 'seeker' &&
      snapshot.self.alive &&
    ...
    if (now - this.scanAt < 100) return;
    this.scanAt = now;
    let count = 0;
    for (const pose of players)
      if (pose.role === 'hider' && pose.alive) {
        for (const region of possibleRegions(
          ...
          let ring = this.rings[count];
          if (!ring) {
            ring = new T.Mesh(this.ringGeometry, this.ringMaterial);
            ring.rotation.x = -Math.PI / 2;
            this.rings.push(ring);
            scene.add(ring);
          }
          ring.visible = true;
          ring.position.set(region.x, 0.045, region.z);
          ring.scale.setScalar(Math.max(0.1, region.radius));
          count++;
        }
      }
    for (let i = count; i < this.rings.length; i++) this.rings[i].visible = false;
  }
  private sync<P extends Vec3 & { id: number }>(
    map: Map<number, T.Mesh>,
    values: P[],
    scene: T.Scene,
    create: () => T.Mesh,
    update: (mesh: T.Mesh, value: P) => void,
  ): void {
    const present = new Set<number>();
    for (const value of values) {
      present.add(value.id);
      let mesh = map.get(value.id);
      if (!mesh) {
        mesh = create();
        map.set(value.id, mesh);
        scene.add(mesh);
      }
      update(mesh, value);
    }
  ...
  clear(): void {
    for (const mesh of [...this.mines.values(), ...this.webs.values(), ...this.rings])
      mesh.removeFromParent();
    this.mines.clear();
    this.webs.clear();
    this.rings = [];
