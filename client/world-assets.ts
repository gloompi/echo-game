...
const loadModel = async (url: string): Promise<T.Object3D> =>
...
function disposeModel(root: T.Object3D): void {
  ...
  root.traverse((object) => {
    if (object instanceof T.Mesh) {
      ...
      const mesh = object as T.Mesh<T.BufferGeometry, T.Material | T.Material[]>;
...
export function mountWorldAsset(
  scene: T.Scene,
  mapId: string,
  fallback: T.Group,
  options: WorldAssetOptions = {},
): WorldAssetHandle {
  const definition = options.asset ?? WORLD_ASSETS[mapId];
  const status: WorldAssetStatus = {
    mapId,
    state: definition ? 'loading' : 'unavailable',
    meshCount: 0,
  };
  if (definition) status.url = definition.url;
  scene.userData.worldAsset = status;
  let disposed = false,
    model: T.Object3D | null = null;
  const ready = (async () => {
    ...
    try {
      ...
      for (const object of embeddedLights) {
