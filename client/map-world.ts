...
function buildMap(layout:GameMap):{scene:T.Scene;animate(time:number):void;dispose():void}{
  ...
  const matrix=new T.Matrix4();for(const items of groups.values()){const first=items[0],material=new T.MeshStandardMaterial({color:first.color,roughness:.8,metalness:.12,emissive:first.emissive,emissiveIntensity:first.emissive?1:0});owned.materials.add(material);const mesh=new T.InstancedMesh(cube,material,items.length);owned.instances.add(mesh);for(let i=0;i<items.length;i++){const b=items[i];matrix.compose(new T.Vector3(b.x,b.y,b.z),new T.Quaternion(),new T.Vector3(b.w,b.h,b.d));mesh.setMatrixAt(i,matrix);}mesh.castShadow=true;mesh.receiveShadow=true;scene.add(mesh);}
