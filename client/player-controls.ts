...
export class PlayerControls {
  ...
  constructor(private notice: (message: string) => void) {
    ...
    this.buttons = {
      jump: panel.querySelector('#bind-jump')!,
      crouch: panel.querySelector('#bind-crouch')!,
    };
    for (const action of ['jump', 'crouch'] as const)
      this.buttons[action].onclick = () => {
        this.waiting = action;
        this.render();
      };
    ...
    window.addEventListener(
      ...
      (e) => {
        ...
        e.preventDefault();
        e.stopImmediatePropagation();
        this.bind(e.deltaY > 0 ? 'WheelDown' : 'WheelUp');
      },
  ...
  private setSensitivity(value: number): void {
    this.sensitivity = Number.isFinite(value) ? Math.max(0.1, Math.min(4, value)) : 1;
  ...
  private bind(code: string): void {
    ...
    if (!next) {
      this.notice('That key is reserved or conflicts with a held action. Choose another key.');
      return;
    }
    this.bindings = next;
    this.waiting = null;
  ...
  private save(): void {
    localStorage.setItem('echo-bindings', JSON.stringify(this.bindings));
    this.clear();
    this.render();
  }
  private render(): void {
    for (const action of ['jump', 'crouch'] as const)
      this.buttons[action].textContent =
        this.waiting === action ? 'PRESS A KEY…' : keyName(this.bindings[action]);
    const hint = document.querySelector('#capture small');
    if (hint)
      hint.textContent = `WASD · ${keyName(this.bindings.jump)} jump · ${keyName(this.bindings.crouch)} crouch / slide · G shield / hook · F mirror`;
    const firstHint = document.querySelector('.control-hint span');
    if (firstHint)
      firstHint.textContent = `${keyName(this.bindings.jump)} Jump / ${keyName(this.bindings.crouch)} Crouch / Slide`;
    const guide = document.querySelectorAll('.key-guide span')[3];
    if (guide) guide.textContent = 'G Shield / Hook · V Mine · B Scan · 1–4 Weapons';
  }
  handles(code: string): boolean {
    const key = normalizeKey(code);
    return fixed.includes(key) || key === this.bindings.jump || key === this.bindings.crouch;
  }
  crouch(keys: Set<string>): boolean {
    return keys.has(this.bindings.crouch);
  }
  jump(keys: Set<string>): boolean {
    ...
    if (this.wheelRelease) {
      this.wheelRelease = false;
      return false;
    }
    if (this.wheelPulses > 0) {
      this.wheelPulses--;
      this.wheelRelease = true;
      return true;
    }
    return false;
  }
  wheel(delta: number): void {
    if (delta && this.bindings.jump === (delta > 0 ? 'WheelDown' : 'WheelUp'))
      this.wheelPulses = Math.min(3, this.wheelPulses + 1);
  }
  clear(): void {
    this.wheelPulses = 0;
    this.wheelRelease = false;
