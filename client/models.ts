...
const ghostOutlineMat = new T.MeshBasicMaterial({
  color: 0x59ffed,
  transparent: true,
  opacity: 0.6,
  depthWrite: false,
...
const ghostRingGeometry = new T.RingGeometry(0.45, 0.48, 32);
const ghostRingMaterial = new T.MeshBasicMaterial({
  color: 0xe657ed,
  transparent: true,
  opacity: 0.65,
  side: T.DoubleSide,
...
const shirts = new Set([0xf4eee7, 0xf3ece5]),
  trousers = new Set([0x39566f, 0x3c556a]);
const armor = new Set([
  0x2e4057, 0x4e6377, 0x4e6277, 0x506d86, 0x6a879e, 0x8dacbe, 0x344962, 0x49627b, 0x405772,
]);
function freezeMeshLocals(root: T.Object3D, except = new Set<T.Object3D>()): void {
  root.traverse((object) => {
    if (object instanceof T.Mesh && !except.has(object)) {
      object.updateMatrix();
...
export function block(
  ...
  const mesh = new T.Mesh(cube, material(color, glow));
  mesh.scale.set(...size);
  mesh.position.set(...pos);
  mesh.castShadow = glow === 0;
  mesh.receiveShadow = true;
  parent.add(mesh);
  if (outlined) {
    const edge = new T.Mesh(cube, outlineMat);
    edge.scale.set(1.055, 1.055, 1.055);
    mesh.add(edge);
  }
  return mesh;
}
export function makeBlaster(): T.Group {
  ...
  block(gun, [0.36, 0.12, 0.62], [0, 0.2, 0.19], 0xffce60, true);
  block(gun, [0.22, 0.2, 0.57], [0, 0.13, 0.7], 0xffb331, true);
  block(gun, [0.24, 0.23, 0.07], [0, 0.13, 1], 0xe4582a, true);
  block(gun, [0.14, 0.12, 0.03], [0, 0.13, 1.045], 0xffe79d, false, 1.5);
  block(gun, [0.23, 0.19, 0.4], [0, -0.14, 0.53], 0xd65b30, true);
  for (let i = 0; i < 3; i++) block(gun, [0.245, 0.16, 0.04], [0, -0.17, 0.42 + i * 0.1], 0x833a32);
  const grip = block(gun, [0.19, 0.34, 0.19], [0, -0.29, -0.05], 0x222a41, true);
  grip.rotation.x = -0.22;
  block(gun, [0.25, 0.23, 0.3], [0, 0, -0.27], 0xe87629, true);
  ...
  muzzle.name = 'muzzle';
  muzzle.position.set(0, 0.13, 1.06);
  gun.add(muzzle);
...
export function makeWeapon(weapon: Weapon = 'blaster'): T.Group {
  const gun = makeBlaster();
  gun.userData.weapon = weapon;
  if (weapon === 'scatter') {
    for (const side of [-1, 1])
      block(gun, [0.12, 0.18, 0.55], [side * 0.2, 0.13, 0.73], 0xba562c, true);
    ...
    for (let i = 0; i < 4; i++)
      block(gun, [0.34, 0.035, 0.06], [0, 0.27, 0.37 + i * 0.12], 0x62eee6, false, 0.7);
  } else if (weapon === 'web') {
    for (const side of [-1, 1]) {
    ...
    for (let i = 0; i < 4; i++)
      block(gun, [0.44, 0.03, 0.32], [0, -0.31 + i * 0.055, 0.3], 0xe1f5ff);
  }
  freezeMeshLocals(gun);
  return gun;
}
const PALETTES: Record<Skin, [number, number]> = {
  ...
  ember: [0xffd8b4, 0x9b422e],
  jade: [0xc6e5cf, 0x267c68],
  violet: [0xe0d2f4, 0x7350a5],
  arctic: [0xf1f8ff, 0x72929f],
  sunset: [0xffdfae, 0xb25373],
  carbon: [0x949fad, 0x283343],
};
export class Character {
  ...
  constructor(
    ...
    if (!seeker) {
      block(this.body, [0.72, 0.65, 0.4], [0, 1.04, 0], 0xf4eee7, true);
      for (let i = 0; i < 3; i++)
        block(this.body, [0.735, 0.105, 0.415], [0, 0.82 + i * 0.22, 0], 0x28303b);
      ...
      block(this.body, [0.33, 0.065, 0.02], [0.08, 1.2, 0.35], 0x48dedb, false, 0.7);
      block(this.body, [0.6, 0.22, 0.12], [0, 0.89, 0.28], 0x243147, true);
      block(this.body, [0.84, 0.13, 0.53], [0, 0.73, 0], 0x8a4f3d, true);
      block(this.body, [0.2, 0.15, 0.04], [0, 0.73, 0.3], 0xa9b4c3);
      block(this.body, [0.42, 0.45, 0.2], [0, 1.15, -0.31], 0x172c42, true);
    }
    for (const side of [-1, 1]) {
      const leg = new T.Group();
      leg.position.set(side * 0.195, 0.67, 0);
      this.body.add(leg);
      this.legs.push(leg);
      block(leg, [0.29, 0.53, 0.32], [0, -0.24, 0], seeker ? 0x495263 : 0x39566f, true);
      if (seeker) block(leg, [0.31, 0.17, 0.1], [0, -0.28, 0.2], 0x233248, true);
      block(leg, [0.32, 0.18, 0.43], [0, -0.57, 0.05], seeker ? 0x152639 : 0xc9bbaa, true);
      block(leg, [0.33, 0.06, 0.44], [0, -0.64, 0.05], 0x162333);
      const arm = new T.Group();
      arm.position.set(side * 0.49, 1.3, 0);
      this.body.add(arm);
      this.arms.push(arm);
    ...
    if (seeker) {
      ...
      block(this.head, [0.6, 0.06, 0.69], [0, 2.16, 0], 0x8dacbe);
      block(this.head, [0.85, 0.64, 0.14], [0, 1.76, -0.33], 0x344962, true);
      for (const side of [-1, 1]) {
        block(this.head, [0.14, 0.46, 0.7], [side * 0.39, 1.82, 0], 0x49627b, true);
      ...
      const mouth = block(this.head, [0.36, 0.13, 0.027], [0.03, 1.505, 0.315], 0x552c32);
      mouth.rotation.z = -0.04;
      block(this.head, [0.3, 0.067, 0.029], [0.03, 1.535, 0.33], 0xfff7e2);
      block(this.head, [0.105, 0.05, 0.03], [-0.25, 1.615, 0.31], 0xd28b77);
    }
    if (!ghost) {
      const shieldGeometry = new T.SphereGeometry(0.94, 16, 12),
        shieldMaterial = new T.MeshBasicMaterial({
          color: 0x60efe4,
          wireframe: true,
          transparent: true,
          opacity: 0.3,
      ...
      this.shield = new T.Mesh(shieldGeometry, shieldMaterial);
      this.shield.position.y = 1.1;
      this.shield.scale.y = 1.35;
      this.shield.visible = false;
      this.group.add(this.shield);
      const bindGeometry = new T.TorusGeometry(0.54, 0.04, 5, 24),
        bindMaterial = new T.MeshBasicMaterial({
          color: 0xe5f3ff,
          transparent: true,
          opacity: 0.9,
          depthWrite: false,
        });
      this.bindRing = new T.Mesh(bindGeometry, bindMaterial);
      this.bindRing.rotation.x = Math.PI / 2;
      this.bindRing.position.y = 0.4;
      this.bindRing.visible = false;
      this.group.add(this.bindRing);
      this.ownedGeometry.push(shieldGeometry, bindGeometry);
      this.ownedMaterials.push(shieldMaterial, bindMaterial);
    }
    if (ghost) {
      this.group.traverse((o) => {
        if (!(o instanceof T.Mesh)) return;
        o.material = o.material === outlineMat ? ghostOutlineMat : ghostBodyMat;
        o.castShadow = false;
        o.receiveShadow = false;
      });
      const ring = new T.Mesh(ghostRingGeometry, ghostRingMaterial);
  ...
  setSkin(skin: Skin = 'classic'): void {
    ...
    this.skin = skin;
    const [light, dark] = PALETTES[skin];
    this.body.traverse((object) => {
      if (!(object instanceof T.Mesh)) return;
      const candidate: unknown = object.material;
      if (!(candidate instanceof T.MeshStandardMaterial)) return;
      let material = candidate;
      const original: unknown = object.userData.originalColor;
      const base = typeof original === 'number' ? original : material.color.getHex();
      if (!shirts.has(base) && !trousers.has(base) && !armor.has(base)) return;
      if (original === undefined) {
        object.userData.originalColor = base;
  ...
  animate(
    ...
    this.setSkin(pose.skin);
    this.setWeapon(pose.weapon);
    if (pose.hitAt !== undefined && pose.hitAt > this.lastHitAt) {
      this.lastHitAt = pose.hitAt;
      if (time * 1000 - pose.hitAt < 650) this.hit();
    }
    this.hitLeft = Math.max(0, this.hitLeft - dt);
    if (this.shield) {
      this.shield.visible = !!pose.shielded;
      this.shield.position.y = pose.crouched ? 0.65 : 1.1;
      this.shield.scale.y = pose.crouched ? 0.75 : 1.35;
    }
    if (this.bindRing) {
    ...
    this.body.position.y = pose.grounded
      ? Math.abs(Math.sin(this.phase)) * 0.04 * walk + Math.sin(time * 2) * 0.009
      : 0;
    this.body.rotation.z = pose.dashing ? 0.1 : Math.sin(this.phase) * walk * 0.022;
    if (this.role === 'seeker') {
      this.arms[0].rotation.set(-1.16, -0.28, -0.16);
      this.arms[1].rotation.set(-1.03, 0.28, 0.18);
      if (this.gun) {
        const pitch = pose.pitch ?? 0;
        this.gun.position.set(
          -0.27,
          eyeHeight(pose.crouched) - 0.33 * Math.cos(pitch) + 0.49 * Math.sin(pitch),
          0.33 * Math.sin(pitch) + 0.49 * Math.cos(pitch),
        );
        this.gun.rotation.x = -pitch;
      }
    } else {
      this.arms[0].rotation.set(-swing * 0.75, 0, -0.08);
      this.arms[1].rotation.set(swing * 0.75, 0, 0.08);
      if (pose.waving) this.arms[0].rotation.set(0.1, 0, -2.45 + Math.sin(time * 12) * 0.24);
    }
    this.body.rotation.x = pose.sliding
      ? -0.42
      : this.hitLeft > 0
        ? -0.16 * Math.sin(this.hitLeft * 30)
        : 0;
    if (pose.sliding) {
      this.legs[0].rotation.x = -1.2;
      this.legs[1].rotation.x = -0.75;
