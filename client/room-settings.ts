...
export class RoomControls {
  ...
  private input<T extends HTMLElement>(id: string): T {
  ...
  constructor(send: (message: ClientMessage) => unknown, notice: (text: string) => void) {
    ...
    const timeline = document.querySelector<HTMLElement>('.timeline-label b');
    if (timeline) timeline.id = 'configured-delay';
    const pitch = document.querySelector('.hero-description strong');
    if (pitch) pitch.textContent = 'the past. You choose how far.';
    const note = document.querySelector('.matchmaking-note');
    if (note) note.textContent = `2–12 players · ${Object.keys(MAPS).length} maps · WebTransport`;
    ...
    const changed = () => {
      this.input('room-settings-note').textContent = 'Unapplied changes. Press APPLY SETTINGS.';
    };
    this.balance = new BalanceEditor(changed);
    this.input('save-room-settings').before(this.balance.element);
    document.getElementById('lobby-note')!.before(this.panel);
    const select = this.input<HTMLSelectElement>('room-map');
    for (const map of Object.values(MAPS)) {
      const option = document.createElement('option');
      option.value = map.id;
      option.textContent = `${map.name} · ${map.half * 2}×${map.half * 2}m`;
      select.append(option);
    }
    this.panel.addEventListener('input', () => {
      this.input<HTMLOutputElement>('delay-output').value = delayLabel(
        Number(this.input<HTMLInputElement>('room-delay').value) * 1000,
      );
      this.preview();
      changed();
    });
    this.input<HTMLButtonElement>('save-room-settings').onclick = () => {
      try {
        const seconds = (id: string) =>
          this.input<HTMLInputElement>(id).value.trim() === ''
            ? NaN
            : Math.round(Number(this.input<HTMLInputElement>(id).value) * 1000);
        const settings = {
          delayMs: seconds('room-delay'),
          roundMs: seconds('room-duration'),
          seekerCount: Number(this.input<HTMLInputElement>('room-seekers').value),
          mapId: select.value,
          reloadMs: seconds('room-reload'),
          dashCooldownMs: seconds('room-dash'),
          mirrorCooldownMs: seconds('room-mirror'),
          bunnyHop: this.input<HTMLSelectElement>('room-bhop').value,
          balance: this.balance.read(),
        };
        if (!validSettings(settings)) {
          notice('Check settings ranges and ability cooldowns.');
          return;
        }
        send({ type: 'settings', settings });
        this.input('room-settings-note').textContent = 'Applying…';
      } catch (error) {
        notice(error instanceof Error ? error.message : 'Invalid settings.');
      }
    };
    this.lobby.id = 'host-return-lobby';
    this.lobby.textContent = 'END ROUND / ROOM SETTINGS';
    ...
    if (brand) brand.href = `/${location.hash}`;
  }
  private preview(): void {
    const map = MAPS[this.input<HTMLSelectElement>('room-map').value as keyof typeof MAPS];
    if (!map) return;
    this.input('map-size').textContent =
      `${map.half * 2} × ${map.half * 2} m · Suggested: ${map.recommended}`;
    this.input('map-description').textContent = map.description;
    const ctx = this.input<HTMLCanvasElement>('map-preview').getContext('2d');
    if (!ctx) return;
    const scale = 224 / (map.half * 2),
      x = (v: number) => 120 + v * scale;
    ctx.fillStyle = '#0b1623';
    ctx.fillRect(0, 0, 240, 240);
    ctx.strokeStyle = '#617587';
    ctx.strokeRect(8, 8, 224, 224);
    for (const b of map.boxes) {
      ctx.fillStyle = b.y > 0 ? '#586575' : '#99a9b3';
      ctx.fillRect(
        x(b.x - b.w / 2),
        x(b.z - b.d / 2),
    ...
    for (const p of map.spawns) {
      ctx.fillStyle = '#72e8be';
      ctx.beginPath();
      ctx.arc(x(p.x), x(p.z), 2, 0, Math.PI * 2);
      ctx.fill();
    }
    for (const p of map.mirrors) {
      ctx.strokeStyle = '#fb77dd';
      ctx.beginPath();
      ctx.arc(x(p.x), x(p.z), 4, 0, Math.PI * 2);
      ctx.stroke();
    }
  }
  private explain(settings: RoomSettings): void {
    const s = resolveSettings(settings),
      p = document.querySelectorAll('#modal-how .rules section p');
    if (p.length === 3) {
  ...
  update(snapshot: Snapshot): void {
    const s = resolveSettings(snapshot.settings),
      key = `${snapshot.room}:${snapshot.settingsVersion}`,
      host = snapshot.self.id === snapshot.host;
    this.panel.disabled = !host || snapshot.phase !== 'lobby';
    this.lobby.hidden = !host;
    if (key !== this.key) {
      this.input<HTMLInputElement>('room-delay').value = String(s.delayMs / 1000);
      this.input<HTMLOutputElement>('delay-output').value = delayLabel(s.delayMs);
      this.input<HTMLInputElement>('room-duration').value = String(s.roundMs / 1000);
      this.input<HTMLInputElement>('room-seekers').value = String(s.seekerCount);
      this.input<HTMLSelectElement>('room-map').value = s.mapId;
      this.input<HTMLSelectElement>('room-bhop').value = s.bunnyHop;
      this.input<HTMLInputElement>('room-reload').value = String(s.reloadMs / 1000);
      this.input<HTMLInputElement>('room-dash').value = String(s.dashCooldownMs / 1000);
      this.input<HTMLInputElement>('room-mirror').value = String(s.mirrorCooldownMs / 1000);
      this.balance.load(s.balance);
      this.key = key;
      this.input('room-settings-note').textContent =
        'Settings applied. Change them between rounds.';
      this.preview();
      this.explain(s);
    }
    const label = document.getElementById('configured-delay');
    if (label) label.textContent = `−${delayLabel(s.delayMs)}`;
  }
}
