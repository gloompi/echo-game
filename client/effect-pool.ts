...
export class CombatEffectPool {
  private beamGeometry = new T.CylinderGeometry(0.017, 0.017, 1, 5);
  private sparkGeometry = new T.BoxGeometry(0.035, 0.035, 0.035);
  private beams = slots(32, this.beamGeometry, 0.9);
  ...
  beam(from: T.Vector3, to: T.Vector3, color: number): void {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      spark.life = spark.max = 0.23;
      spark.mesh.position.copy(to);
      spark.mesh.scale.setScalar(1);
      spark.mesh.quaternion.identity();
      spark.velocity.set((Math.random() - 0.5) * 3, Math.random() * 3, (Math.random() - 0.5) * 3);
      this.activate(spark);
    }
  }
  ...
  private updateSlot(slot: Slot, dt: number, gravity: boolean): void {
    ...
    slot.life -= dt;
    slot.material.opacity = Math.max(0, slot.life / slot.max);
    if (gravity) {
      slot.mesh.position.addScaledVector(slot.velocity, dt);
      slot.velocity.y -= dt * 5;
    }
    if (slot.life <= 0) {
      slot.active = false;
      slot.mesh.visible = false;
