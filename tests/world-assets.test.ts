...
function setup() {
  const scene = new T.Scene(), fallback = new T.Group(), mirrors = new T.Group(), light = new T.HemisphereLight();
...
test('maps without authored art retain their procedural world without a request', async () => {
  const { scene, fallback } = setup();
  const asset = mountWorldAsset(scene, 'unconfigured-test-map', fallback, { load: async () => { throw new Error('Must not request a model.'); } });
  await asset.ready;
...
test('a loaded world replaces only fallback art and releases its shared resources once', async () => {
  const { scene, fallback, mirrors, light } = setup(), loaded = model();
  const exportedLight = new T.PointLight(); loaded.root.add(exportedLight);
  const asset = mountWorldAsset(scene, 'test-map', fallback, { asset: { url: '/world.glb' }, load: async url => {
    assert.equal(url, '/world.glb'); return loaded.root;
  } });
...
test('an arena disposed during a request never attaches the late model', async () => {
  const { scene, fallback } = setup(), loaded = model();
  let resolve!: (root: T.Object3D) => void;
  const request = new Promise<T.Object3D>(done => { resolve = done; });
  const asset = mountWorldAsset(scene, 'old-map', fallback, { asset: { url: '/old.glb' }, load: () => request });
  asset.dispose();
  resolve(loaded.root); await asset.ready;
...
test('failed and empty exports keep the visible fallback and expose the failure', async t => {
  t.mock.method(console, 'warn', () => {});
  for (const load of [async () => { throw new Error('Network unavailable'); }, async () => new T.Group()]) {
    const { scene, fallback } = setup();
    const asset = mountWorldAsset(scene, 'test-map', fallback, { asset: { url: '/world.glb' }, load });
    await asset.ready;
