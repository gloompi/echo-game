...
test('two clients load and play both authored Blender worlds', async ({ browser }, info) => {
  ...
  try {
    const host = await a.newPage(), friend = await b.newPage(), errors: string[] = [];
    ...
    for (const [roundIndex, [id, name]] of [['mirror-yard', 'Mirror Yard'], ['neon-carnival', 'Neon Carnival']].entries()) {
      ...
      for (const page of [host, friend]) {
        ...
        await expect(page.locator('#game')).toHaveAttribute('data-world-asset', 'loaded', { timeout: 30_000 });
        expect(Number(await page.locator('#game').getAttribute('data-world-meshes'))).toBeGreaterThan(30);
      }
      ...
      await expect(friend.locator('#phase-label')).toHaveText(`ROUND ${String(roundIndex + 1).padStart(2, '0')}`, { timeout: 15_000 });
      ...
      await expect(host.locator('#phase-label')).toHaveText(`ROUND ${String(roundIndex + 1).padStart(2, '0')}`);
