...
function repository(context, inherited = process.env) {
  ...
  context.after(() => rmSync(scratch, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 }));
  ...
  const env = {
    ...
    CI: '', GIT_CONFIG_GLOBAL: globalConfig, GIT_CONFIG_NOSYSTEM: '1', GIT_TEMPLATE_DIR: templates,
  };
  /** @param {string[]} args */
  const git = (...args) => {
...
test('fixture commands cannot inherit the caller repository, index, or config overrides', context => {
  ...
  const inherited = {
    ...process.env,
    GIT_DIR: join(foreign.root, '.git'), GIT_WORK_TREE: foreign.root, GIT_INDEX_FILE: indexPath,
    GIT_CONFIG_GLOBAL: '\\\\.\\nul', GIT_CONFIG: configPath,
    GIT_CONFIG_COUNT: '1', GIT_CONFIG_KEY_0: 'core.bare', GIT_CONFIG_VALUE_0: 'true',
    git_config_parameters: "'core.bare=true'", CI: 'true', ci: 'true',
  };
  ...
  for (const key of Object.keys(inherited).filter(key => /^(GIT_|CI$)/i.test(key))) {
    if (key !== 'CI' && key !== 'GIT_CONFIG_GLOBAL' && key !== 'GIT_CONFIG_NOSYSTEM' && key !== 'GIT_TEMPLATE_DIR') {
...
test('checkedRevision uses the supplied environment for every Git subprocess', context => {
  ...
  `, root], {
    cwd: root, encoding: 'utf8', input: JSON.stringify(env),
...
test('real hook callers still inherit their explicitly selected Git index by default', context => {
  ...
  const added = spawnSync('git', ['add', 'game.txt'], { cwd: root, env: alternateEnv, encoding: 'utf8' });
  ...
  `, root], { cwd: root, env: alternateEnv, encoding: 'utf8' });
  assert.equal(result.status, 0, result.error?.message || result.stderr);
