...
test('production imports may only point inward', () => {
  for (const [file, specifier] of [
    ['shared/state.ts', '../client/main.js'],
    ['shared/state.ts', 'three'],
    ['client/network/state.ts', '../../tests/helper.js'],
    ['client/main.ts', 'node:fs'],
    ['client/main.ts', 'fs'],
    ['client/main.ts', '../scripts/play.mjs'],
    ['client/main.ts', '/tmp/module.js'],
    ['shared/state.ts', '../../shared/outside.js'],
  ])
...
test('declared runtime and domain dependencies remain allowed', () => {
  for (const [file, specifier] of [
    ['client/main.ts', 'three'],
    ['client/main.ts', 'three/addons/loaders/GLTFLoader.js'],
    ['client/network/state.ts', '../../shared/types.js'],
    ['shared/state.ts', './types.js'],
    ['client/main.ts', './style.css'],
    ['tests/game.test.ts', '../client/main.js'],
  ])
...
test('imports, re-exports, dynamic imports, and type imports share the boundary policy', () => {
  ...
  assert.equal(
    lintSource('shared/invalid.ts', source).filter((v) => v.rule === 'architecture').length,
    5,
  );
});
...
test('unsafe syntax is flagged without confusing strings for code', () => {
  const rules = lintSource('scripts/bad.mjs', 'var x = 1; debugger; eval(x); new Function(x);').map(
    (v) => v.rule,
  );
...
test('typecheck bypasses are found only in real comment tokens', () => {
  assert.equal(
    lintSource('client/bad.ts', '// @ts-nocheck\nconst x = 1;')[0]?.rule,
    'no-typecheck-bypass',
  );
