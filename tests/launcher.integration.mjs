...
test('sharing refuses missing, loopback and unsafe UDP endpoint configuration before launching services', { timeout: 10000 }, async () => {
  for (const endpoint of ['', 'https://127.0.0.1:4433/echo', 'http://udp.example/echo', 'https://udp.example/echo?key=secret']) {
    const child = spawn(process.execPath, [resolve('scripts/play.mjs'), '--share'], {
      env: { ...process.env, ECHO_WT_PUBLIC_URL: endpoint }, windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'],
    });
    let output = ''; child.stdout.on('data', chunk => { output += chunk; }); child.stderr.on('data', chunk => { output += chunk; });
    const code = await new Promise((resolve, reject) => { child.once('error', reject); child.once('exit', resolve); });
    assert.notEqual(code, 0); assert.match(output, /Sharing requires ECHO_WT_PUBLIC_URL/);
...
test('share launcher registers public URL, prints keyed invites and stops children (mock services)', { skip: process.platform === 'win32', timeout: 10000 }, async () => {
  ...
  const child = spawn(process.execPath, [resolve('scripts/play.mjs'), '--share'], { env: { ...process.env, PATH: `${dir}:${process.env.PATH}`, PORT: String(port), ECHO_SERVER_BIN: binary, ECHO_ACCESS_KEY: '', ECHO_WT_PUBLIC_URL: 'https://udp.example:4433/echo' }, stdio: ['ignore', 'pipe', 'pipe'] });
  let output = ''; child.stdout.on('data', c => { output += c; }); child.stderr.on('data', c => { output += c; });
  try {
    for (let i = 0; i < 100 && !output.includes('FRONTEND INVITE:'); i++) await sleep(30);
    assert.match(output, /FRONTEND INVITE: https:\/\/echo-mock\.trycloudflare\.com\/#key=[a-f0-9]{48}/);
