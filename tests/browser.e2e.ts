...
test('two browsers synchronize host settings, play, and return to the lobby', async ({
  browser,
}) => {
  ...
  try {
    ...
    await host.locator('#room-delay').evaluate((node: HTMLInputElement) => {
      node.value = '1.25';
    ...
    await expect(friend.locator('#room-delay')).toHaveValue('1.25');
    ...
    await expect(friend.locator('#configured-delay')).toHaveText('−1.25s');
...
test('real seeker WebTransport feed excludes current Hiders during warmup', async ({ page }) => {
  await page.goto('/');
  const peer = await connectPeer(page, {
    type: 'join',
    mode: 'practice',
    name: 'ProtocolTest',
    preference: 'seeker',
  ...
  try {
    ...
    const later = await peer.wait(
      (s: Snapshot) => s.type === 'snapshot' && s.players.some((p) => p.role === 'hider'),
    );
