...
test('hider skin, host ability tuning and shield activate through the real game client', async ({
  page,
}, info) => {
  const errors: string[] = [];
  ...
  await page.locator('[data-role="hider"]').click();
  await page.locator('#create-room').click();
  const skin = page.locator('#lobby').getByLabel('Character skin');
  await expect(skin.locator('option')).toHaveCount(8);
  await skin.selectOption('jade');
  ...
  try {
    await expect(page.locator('#combat-hud')).toContainText('G SHIELD · ACTIVE');
  } finally {
...
test('seeker switches all four weapon models and activates the possibility scan', async ({
  page,
}, info) => {
  const errors: string[] = [];
  page.on('pageerror', (error) => errors.push(error.message));
  await page.goto('/');
  await expect(page.locator('#loading')).toBeHidden();
  await page.locator('[data-role="seeker"]').click();
  ...
  for (const [key, weapon] of [
    ...
    try {
      await expect(page.locator('#combat-hud')).toContainText(`1–4 ${weapon}`);
    } finally {
  ...
  try {
    await expect(page.locator('#combat-hud')).toContainText('B SCAN · ACTIVE');
  } finally {
