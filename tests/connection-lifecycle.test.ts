...
class FakeTransport implements GameTransport {
  ...
  connect(
    _url: string,
    message: (value: ServerMessage) => void,
    closed: () => void,
  ): Promise<void> {
  ...
  sendLatest(text: string): boolean {
...
void test('synchronous transport factory errors reach the failure callback', () => {
...
void test('synchronous adapter connect errors close the partially opened session', () => {
...
void test('an unsuccessful join fails immediately, even below the congestion limit', async () => {
  const wire = new FakeTransport();
  wire.accept = false;
  const failures: string[] = [];
  const connection = new Connection(
...
void test('a synchronous close during join cannot install an orphan keepalive', async (context) => {
...
void test('explicit close is idempotent and late callbacks cannot fail the next room', async () => {
  const wire = new FakeTransport();
  const failures: string[] = [];
  const connection = new Connection(
    () => {},
...
void test('explicit join keys take precedence over invitation fragments', async () => {
  const wire = new FakeTransport();
  const connection = new Connection(
    () => {},
    () => {},
  ...
  try {
    ...
    assert.equal(sent.accessKey, 'explicit');
  } finally {
    connection.close();
  }
});

void test('invitation discovery cannot overwrite a newer connection generation', async () => {
...
void test('invalid discovery URLs retain the last usable invitation endpoint', async () => {
  const connection = new Connection(
    () => {},
    () => {},
    () => {},
    () => new FakeTransport(),
    {
      ...environment,
      fetch: () =>
        Promise.resolve(new Response(JSON.stringify({ publicUrl: 'javascript:alert(1)' }))),
    },
  );
  assert.equal(
    await connection.invite('ABC234'),
    'http://localhost:3000/?room=ABC234#key=fragment',
  );
  assert.equal(connection.publicUrl, null);
  connection.close();
});
