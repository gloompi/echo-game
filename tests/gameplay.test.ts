...
test('every selectable map has authoritative data and valid room settings', () => {
  assert.deepEqual(Object.keys(MAPS).sort(),[...MAP_IDS].sort());
  for(const id of MAP_IDS){assert.equal(MAPS[id].id,id);assert.ok(validSettings({...options,mapId:id}));}
});
for(const layout of Object.values(MAPS)) {
  test(`${layout.name}: spawns, mirror exits and mirror pairs are valid`, () => {
    ...
    for(const b of layout.boxes){
      ...
      assert.ok([b.x,b.y,b.z,b.w,b.h,b.d].every(Number.isFinite),`${layout.name}: non-finite collider ${b.id}`);
      assert.ok(b.w>0&&b.h>0&&b.d>0,`${layout.name}: empty collider ${b.id}`);
      assert.ok(Math.abs(b.x)+b.w/2<=layout.half+1e-8&&Math.abs(b.z)+b.d/2<=layout.half+1e-8,`${layout.name}: out-of-bounds collider ${b.id}`);
      assert.ok(['cover','platform','step','reactor'].includes(b.kind),`${layout.name}: unknown collider kind ${b.id}`);
    }
    assert.equal(layout.spawns.length,12);assert.ok(layout.waypoints.length>0);
    for(const p of [...layout.spawns,...layout.waypoints,...layout.mirrors,...layout.mirrors.map(m=>m.exit)]){
      assert.ok([p.x,p.y,p.z].every(Number.isFinite),`${layout.name}: non-finite route point`);
      assert.ok(Math.abs(p.x)<layout.half&&Math.abs(p.z)<layout.half,`${layout.name}: out-of-bounds route point`);
    }
    ...
    for(const mirror of layout.mirrors){assert.ok(Number.isFinite(mirror.yaw));assert.notEqual(mirror.id,mirror.target);const target=layout.mirrors.find(m=>m.id===mirror.target);assert.ok(target);assert.equal(target.target,mirror.id);}
...
test('authored world stair routes reach their rooftops without jumping', () => {
  for(const route of [
    ...
    const layout=MAPS[route.id],state=makeMotor(route.start),settings={...options,mapId:route.id};
    assert.ok(canOccupy(route.start,CFG.height,layout.boxes));
    for(let i=0;i<100;i++)move(state,input({mz:route.mz}),'hider',CFG.dt,layout.boxes,layout.half,settings);
    assert.ok(state.grounded);assert.ok(Math.abs(state.y-route.roof)<1e-8,`${layout.name}: rooftop was not reached`);
    assert.ok(canOccupy(state,CFG.height,layout.boxes));
  }
});

test('Mirror Yard service tunnel blocks standing but permits a complete crouched passage', () => {
  const layout=MAPS['mirror-yard'],state=makeMotor({x:0,y:0,z:-14}),settings={...options,mapId:layout.id};
  const walk=(n:number,controls:Partial<ReturnType<typeof neutralInput>>)=>{for(let i=0;i<n;i++)move(state,input(controls),'hider',CFG.dt,layout.boxes,layout.half,settings);};
  walk(40,{mz:1});assert.ok(state.z>-15.8);
  walk(100,{mz:1,crouch:true});assert.ok(state.z<-18.5);
  walk(20,{});assert.equal(state.crouched,false);assert.ok(canOccupy(state,CFG.height,layout.boxes));
});

test('Neon Carnival tunnel has a complete crouched passage in both directions', () => {
  const layout=MAPS['neon-carnival'],ceiling=layout.boxes.find(b=>b.id==='tunnel-crouch-ceiling')!,settings={...options,mapId:layout.id};
  for(const route of [{z:-19,mz:-1},{z:-4,mz:1}]){
    const state=makeMotor({x:-21,y:0,z:route.z});
    const walk=(n:number,controls:Partial<ReturnType<typeof neutralInput>>)=>{for(let i=0;i<n;i++)move(state,input(controls),'hider',CFG.dt,layout.boxes,layout.half,settings);};
    walk(30,{mz:route.mz});const blocked=state.z;walk(60,{mz:route.mz});assert.ok(Math.abs(state.z-blocked)<1e-8);
    walk(300,{mz:route.mz,crouch:true});
    assert.ok(route.mz<0?state.z>ceiling.z+ceiling.d/2+CFG.radius:state.z<ceiling.z-ceiling.d/2-CFG.radius,'Tunnel exit is blocked');
    walk(20,{});assert.equal(state.crouched,false);assert.ok(state.grounded);assert.ok(canOccupy(state,CFG.height,layout.boxes));
  }
});

test('Neon Carnival side stairs connect to the tunnel roof without blocking its ground passage', () => {
  const layout=MAPS['neon-carnival'],state=makeMotor({x:-17,y:0,z:1.2}),settings={...options,mapId:layout.id};
  for(let i=0;i<82;i++)move(state,input({mz:1}),'hider',CFG.dt,layout.boxes,layout.half,settings);
  assert.ok(Math.abs(state.y-4.2)<1e-8);assert.ok(state.grounded);
  for(let i=0;i<36;i++)move(state,input({mx:-1}),'hider',CFG.dt,layout.boxes,layout.half,settings);
  assert.ok(state.x<-20);assert.ok(Math.abs(state.y-4.2)<1e-8);assert.ok(state.grounded);assert.ok(canOccupy(state,CFG.height,layout.boxes));
});
