...
class FakeTransport implements GameTransport {
  bufferedAmount=0; sent:string[]=[]; url=''; closed=0; deliver: (message:ServerMessage)=>void=()=>{}; disconnected:()=>void=()=>{}; accept=true;
  async connect(url:string,message:(value:ServerMessage)=>void,closed:()=>void):Promise<void>{this.url=url;this.deliver=message;this.disconnected=closed;}
