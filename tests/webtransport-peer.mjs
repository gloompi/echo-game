...
export async function connectPeer(page, join) {
  const id = await page.evaluate(async (joinMessage) => {
    ...
    const transport = new WebTransport(config.loopbackUrl || config.url, {
      serverCertificateHashes: config.certificateHashes.map((value) => ({
        algorithm: 'sha-256',
    ...
    peer.send = (message) => {
      const body = new TextEncoder().encode(JSON.stringify(message)),
        bytes = new Uint8Array(4 + body.length);
      ...
      try {
        for (;;) {
          ...
          while (buffer.length >= 4) {
            ...
            peer.messages.push(
              JSON.parse(
                new TextDecoder('utf-8', { fatal: true }).decode(buffer.subarray(4, 4 + length)),
              ),
            );
    ...
    const failed = (error) => {
      if (!peer.closed) peer.error = String(error);
    };
    ...
    void (async () => {
      ...
      try {
        for (;;) {
          ...
          if (item.done) break;
          void read(item.value).catch(failed);
        }
      } finally {
        reader.releaseLock();
      }
  ...
  return {
    send: (message) =>
      page.evaluate(({ id, message }) => globalThis.echoTestPeers[id].send(message), {
        id,
        message,
      }),
    ...
    wait: async (predicate) => {
      for (let i = 0; i < 240; i++) {
        const state = await page.evaluate((id) => {
          const p = globalThis.echoTestPeers[id];
          return { messages: p.messages, closed: p.closed, error: p.error };
        }, id);
