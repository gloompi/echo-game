...
test('host can select each map and configure reload, dash, mirrors and hopping', async ({
  page,
}) => {
  const errors: string[] = [];
  ...
  await expect(page.locator('#room-settings')).toBeVisible();
  for (const [id, name] of [
    ...
    await page.locator('#room-reload').fill('0');
    await page.locator('#room-dash').fill('1.2');
    await page.locator('#room-mirror').fill('60');
    await page.locator('#room-bhop').selectOption('auto');
    ...
    await page.locator('#host-return-lobby').click();
