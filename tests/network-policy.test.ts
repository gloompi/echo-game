...
void test('clock uses snapshots until a valid pong provides an RTT estimate', () => {
  let localNow = 100;
...
void test('clock rejects invalid, future, and stale pong timestamps', () => {
  const clock = new ServerClock(() => 4_000);
  for (const [sentAt, serverNow] of [
    [NaN, 10],
    [0, Infinity],
    [4_001, 10],
    [1_000, 10],
...
void test('clock smooths valid measurements and reset removes the previous server epoch', () => {
...
void test('snapshot cursor rejects duplicates, old frames, and non-finite times', () => {
...
void test('invitations preserve the fragment and path but replace all query parameters', () => {
  const url = createInviteUrl(
    'ABC234',
    { origin: 'https://local.example', hash: '#key=a%26b' },
...
void test('local invitations work without a public URL or access key', () => {
  assert.equal(
    createInviteUrl('ABC234', { origin: 'http://127.0.0.1:3000', hash: '' }, null),
    'http://127.0.0.1:3000/?room=ABC234',
  );
  assert.equal(fragmentAccessKey('#unrelated=value'), undefined);
});

void test('invitations reject non-web protocols and embedded credentials', () => {
...
void test('public URL discovery reads unknown JSON without trusting a cast', () => {
...
void test('transport discovery accepts the server null loopback URL representation', () => {
...
void test('transport discovery rejects malformed unknown JSON', () => {
  for (const transport of [
    null,
    false,
    [],
    {},
    { url: 42, protocolVersion: 3 },
...
void test('local hosts use loopback while remote players retain the public endpoint', () => {
  const config = {
    url: 'https://public.example/echo',
    protocolVersion: 3,
...
void test('transport configuration validates protocol, scheme, credentials, and fragments', () => {
  for (const url of [
    'http://host/echo',
    'https://user:secret@host/echo',
    'https://host/echo#key',
  ]) {
    assert.throws(() => resolveTransportConfig({ url, protocolVersion: 3 }, undefined));
  }
  assert.throws(() =>
    resolveTransportConfig({ url: 'https://host/echo', protocolVersion: 2 }, undefined),
  );
  assert.throws(() =>
    resolveTransportConfig(
      {
        url: 'https://host/echo',
        protocolVersion: 3,
        loopbackUrl: 'https://attacker.example/echo',
      },
      'localhost',
    ),
  );
...
void test('certificate pins must be at most two complete SHA-256 byte arrays', () => {
  ...
  for (const hashes of [
    ...
    assert.throws(() =>
      resolveTransportConfig({ ...config, certificateHashes: hashes }, undefined),
    );
  }
});
