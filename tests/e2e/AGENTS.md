...
Tracing is owned by Playwright Test configuration for all contexts. Keep manual context cleanup and screenshots, but do not start a second trace or throw cleanup failures from a finally block that would hide the original test error.
